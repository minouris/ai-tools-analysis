...
**Tool Version:** v2.5 (as of February 2026)  
...
**Official Documentation:** https://docs.cursor.com  
**Version Analysed:** v2.5 (as of February 2026)  
...
**Version:** Cursor is now at approximately v2.5 (released February 17–18, 2026).

**Plugins and Marketplace:** Plugins now package skills, subagents, and hooks, and are distributed via the Cursor Marketplace. Initial launch partners include Amplitude, AWS, Figma, Linear, and Stripe.

**Async Subagents:** Subagents can now be launched asynchronously, enabling the Cursor agent to work on multiple tasks in parallel, including multi-file and larger feature implementations.

**Cursor Blame (Enterprise):** A new AI-based code attribution feature that identifies which lines of code were generated by AI, available on Enterprise plans.

**Citation:** Cursor Changelog. Cursor. https://cursor.com/changelog. Accessed 21 February 2026.
...
Cursor inherits the familiar VS Code interface and extension ecosystem whilst adding powerful AI-native features. It enables developers to write, edit, and understand code through conversational AI interactions, with the AI having deep understanding of the entire codebase context. From v2.5, Cursor supports a plugin and marketplace ecosystem that bundles skills, subagents, and hooks, allowing teams to extend and customise the agent's capabilities.
...
- **Plugins and Marketplace**: Plugins bundle skills, subagents, and hooks; available via the Cursor Marketplace with initial partners including Amplitude, AWS, Figma, Linear, and Stripe (v2.5)
- **Async Subagents**: Subagents launched asynchronously enable the agent to tackle multi-file and larger features in parallel (v2.5)
- **Cursor Blame (Enterprise)**: AI-based code attribution showing which lines were generated by AI and the context of changes (v2.5)

**Citation:** Official information available at https://www.cursor.sh and https://docs.cursor.com. Accessed 21 February 2026. Cursor Changelog. Cursor. https://cursor.com/changelog. Accessed 21 February 2026.
...
The following CLI and workflow improvements have been made since January 2026.

**CLI Model Selection and Rule Management (January 2026):** The CLI now supports selecting models, managing instruction rules with the `/rules` command (create and edit rules), and adding or removing MCP servers with `/mcp enable` and `/mcp disable`.

**CLI Chat Tab Naming:** Chat tabs are now automatically named in the CLI. The CLI agent command is now the default entry point.

**CLI Improvements in v2.5:** The CLI can now hand off AI-generated plans to the cloud or execute them locally. A persistent decision menu with new keyboard shortcuts has also been introduced.

**Mermaid ASCII Diagrams in CLI:** Mermaid code blocks now render as ASCII diagrams in the CLI, supporting flowcharts, sequence diagrams, state machines, class diagrams, and ER diagrams. Users can switch between ASCII and original rendering using Ctrl+O.
...
- **Async Subagents**: Subagents can be launched asynchronously to tackle multi-file and larger features in parallel (v2.5)

**CLI Capabilities (v2.5):**

The Cursor CLI provides a command-line interface to the agent with the following capabilities:
- Model selection via command-line flags
- `/rules` command: manage and generate instruction rules directly from the CLI
- `/mcp enable` and `/mcp disable` commands for enabling and disabling MCP servers from the CLI
- Chat tabs are automatically named
...
The following IDE and environment integration changes have been made since January 2026.

**Hooks for Security and Platform Teams (January 2026):** Hooks have been added to allow enterprise security and platform teams to observe, block, or extend agent behaviour for compliance and governance purposes.

**Sandbox Network Access Controls (v2.5):** Granular controls are now available to restrict which domains agents can access in sandboxed commands. Admin features allow organisations to enforce access policies across their teams.
...
**CLI Available:** Yes (Cursor CLI, v2.5)

**Installation:** Included with Cursor desktop application

Cursor provides a CLI agent as the default entry point for interacting with the AI from the terminal. The CLI supports model selection, rule management, and MCP server control, in addition to plan execution and handoff workflows.

**Available CLI Commands:**

| Command | Description |
|---------|-------------|
| CLI agent (default) | Start an agent session from the terminal |
| `/rules` | Manage and generate instruction rules |
...
- Chat tabs are automatically named
- AI-generated plans can be handed off to the cloud or executed locally
- Persistent decision menu with keyboard shortcuts for plan management
- Mermaid code blocks rendered as ASCII diagrams (flowcharts, sequence diagrams, state machines, class diagrams, ER diagrams); toggle between ASCII and original Mermaid source with Ctrl+O
...
### 7.6 Security, Compliance, and Enterprise Features

**Hooks for Security Teams (January 2026):**

Hooks are available for enterprise security and platform teams to observe, block, or extend agent behaviour for compliance and governance purposes. Integration with ecosystem partners supports secrets management and code safety workflows, enabling organisations to enforce security policies on agent actions.

**Sandbox Network Access Controls (v2.5):**

Granular controls are available to restrict which domains agents can access in sandboxed command execution. Admin features allow organisations to enforce access policies across their teams, ensuring agents operate within approved network boundaries.
...
3. Cursor Changelog - https://cursor.com/changelog. Accessed 21 February 2026.
4. Cursor Community Resources - Various community forums and discussions

### Version Information

- **Tool Version Analysed:** v2.5 (as of February 2026)
...
This section summarises all changes to Cursor identified since the original analysis date of 20 January 2026.
...
- **Version:** Cursor is now at approximately v2.5 (released February 17–18, 2026).
- **Plugins and Marketplace:** Plugins now package skills, subagents, and hooks, distributed via the Cursor Marketplace. Initial launch partners include Amplitude, AWS, Figma, Linear, and Stripe.
- **Async Subagents:** Subagents can now be launched asynchronously for parallel multi-file and feature work.
- **Cursor Blame (Enterprise):** New AI-based code attribution identifying AI-generated lines, available on Enterprise plans.

### Application Development Workflow Changes

- **CLI Model Selection and Rule Management (January 2026):** CLI now supports model selection, the `/rules` command (create and edit rules), and `/mcp enable` / `/mcp disable` for MCP server management.
- **CLI Chat Tab Naming:** Chat tabs are now automatically named; the CLI agent command is the new default entry point.
- **CLI Handoff (v2.5):** AI-generated plans can be handed off to the cloud or executed locally, with a persistent decision menu and new keyboard shortcuts.
- **Mermaid ASCII Diagrams:** Mermaid code blocks now render as ASCII diagrams in the CLI (flowcharts, sequence diagrams, state machines, class diagrams, ER diagrams); toggle with Ctrl+O.

### IDE and Environment Integration Changes

- **Hooks for Security and Platform Teams (January 2026):** Enterprise hooks for observing, blocking, or extending agent behaviour for security and compliance.
- **Sandbox Network Access Controls (v2.5):** Granular domain access controls for sandboxed agent commands, with admin enforcement features for organisations.
...
| 21 February 2026 | 1.1 | Added changes since January 2026: v2.5 features including Plugins and Marketplace, Async Subagents, Cursor Blame (Enterprise), CLI improvements (model selection, rule management, MCP management, chat tab naming, plan handoff), Mermaid ASCII diagrams in CLI, Hooks for security teams, and Sandbox Network Access Controls | GitHub Copilot |
